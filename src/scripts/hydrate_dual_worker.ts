import { createClient } from '@supabase/supabase-js';
import dotenv from 'dotenv';
import path from 'path';
import { fileURLToPath } from 'url';
import OpenAI from 'openai';
import axios from 'axios';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
dotenv.config({ path: path.resolve(__dirname, '../../.env') });

const supabaseUrl = process.env.VITE_SUPABASE_URL || '';
const supabaseKey = process.env.VITE_SUPABASE_ANON_KEY || process.env.SUPABASE_SERVICE_KEY || '';
const TMDB_READ_TOKEN = 'eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI4ZGEyZDgzOTlmM2FmOThjNGQyOTBiMmFhYWNjNjAyZSIsIm5iZiI6MTc2NDU2MzAzMi45ODMsInN1YiI6IjY5MmQxODU4ZWQzYTc0MTRiMjUxMWY3YSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.xUHn1pyh2vlISckJdCh-_r0-Rzx3HCr5xgS1DtK322U';
const openaiKey = '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';

const supabase = createClient(supabaseUrl, supabaseKey);
const openai = new OpenAI({ apiKey: openaiKey });

const MAX_AI_OPTIMIZATIONS = 4000;
let globalAiCount = 0; // Shared counter across workers

async function sleep(ms: number) {
    return new Promise(resolve => setTimeout(resolve, ms));
}

async function fetchTmdbPerson(tmdbId: string) {
    try {
        const id = tmdbId.replace('tmdb-person-', '');
        const url = `https://api.themoviedb.org/3/person/${id}`;
        const res = await axios.get(url, {
            headers: {
                accept: 'application/json',
                Authorization: `Bearer ${TMDB_READ_TOKEN}`
            },
            timeout: 10000
        });
        return res.data;
    } catch (e) {
        return null;
    }
}

async function optimizeBio(name: string, rawBio: string, knownFor: string) {
    try {
        const prompt = `Rewrite the biography for "${name}" (Known for: ${knownFor}). Make it engaging, professional, and SEO-friendly (100-200 words). Do not invent facts.

Raw Bio: "${rawBio.substring(0, 1000)}..."`;

        const completion = await openai.chat.completions.create({
            messages: [{ role: "user", content: prompt }],
            model: "gpt-4o-mini",
            max_tokens: 350,
        });

        return completion.choices[0].message.content || rawBio;
    } catch (e) {
        console.error(`AI Error for ${name}:`, e);
        return rawBio;
    }
}

async function processWorker(workerNum: number, personIds: string[]) {
    console.log(`\n[Worker ${workerNum}] Starting with ${personIds.length} records`);

    let aiCount = 0;
    let rawCount = 0;
    let failedCount = 0;

    for (let i = 0; i < personIds.length; i++) {
        const personId = personIds[i];

        // Fetch person from DB
        const { data: person, error: fetchErr } = await supabase
            .from('cast')
            .select('*')
            .eq('id', personId)
            .single();

        if (fetchErr || !person) {
            process.stdout.write('e');
            continue;
        }

        const tmdbId = person.tmdb_id;

        if (!tmdbId || !tmdbId.startsWith('tmdb-person-')) {
            await supabase.from('cast').update({ biography: '' }).eq('id', personId);
            process.stdout.write('s');
            continue;
        }

        const tmdbData = await fetchTmdbPerson(tmdbId);
        await sleep(60); // Slightly slower with 2 workers to be safe

        if (!tmdbData) {
            await supabase.from('cast').update({ biography: '', updated_at: new Date().toISOString() }).eq('id', personId);
            failedCount++;
            process.stdout.write('x');
            continue;
        }

        const rawBio = tmdbData.biography || '';
        const knownFor = tmdbData.known_for_department || 'Acting';
        let finalBio = rawBio;

        // AI optimize if budget available and bio long enough
        if (globalAiCount < MAX_AI_OPTIMIZATIONS && rawBio.length > 50) {
            finalBio = await optimizeBio(tmdbData.name, rawBio, knownFor);
            globalAiCount++;
            aiCount++;
            rawCount++;
            process.stdout.write('✨');
        } else {
            rawCount++;
            process.stdout.write('.');
        }

        const { error: updateErr } = await supabase
            .from('cast')
            .update({
                biography: finalBio,
                birthday: tmdbData.birthday,
                place_of_birth: tmdbData.place_of_birth,
                deathday: tmdbData.deathday,
                image_url: tmdbData.profile_path || person.image_url,
                updated_at: new Date().toISOString()
            })
            .eq('id', personId);

        if (updateErr) {
            console.error(`\n[W${workerNum}] Update failed:`, updateErr);
        }

        // Progress update every 100 records
        if ((i + 1) % 100 === 0) {
            console.log(`\n[Worker ${workerNum}] Progress: ${i + 1}/${personIds.length} | AI: ${aiCount}, Raw: ${rawCount}, Failed: ${failedCount}`);
        }
    }

    console.log(`\n[Worker ${workerNum}] COMPLETE | AI: ${aiCount}, Raw: ${rawCount}, Failed: ${failedCount}`);
}

async function main() {
    console.log('🚀 Starting Dual-Worker Hydration...');
    console.log(`Max AI Optimizations: ${MAX_AI_OPTIMIZATIONS}`);

    // 1. Fetch all NULL bio IDs
    console.log('\n📋 Fetching all NULL biography records...');
    const { data: nullBioRecords, error } = await supabase
        .from('cast')
        .select('id')
        .is('biography', null);

    if (error || !nullBioRecords) {
        console.error('Failed to fetch NULL records:', error);
        process.exit(1);
    }

    console.log(`Found ${nullBioRecords.length} records to process`);

    if (nullBioRecords.length === 0) {
        console.log('✅ No NULL biographies found. All done!');
        return;
    }

    // 2. Split into 2 sets
    const ids = nullBioRecords.map(r => r.id);
    const midpoint = Math.floor(ids.length / 2);
    const worker1Ids = ids.slice(0, midpoint);
    const worker2Ids = ids.slice(midpoint);

    console.log(`\nWorker 1: ${worker1Ids.length} records`);
    console.log(`Worker 2: ${worker2Ids.length} records`);

    // 3. Run both workers in parallel
    console.log('\n🚀 Starting workers...\n');
    await Promise.all([
        processWorker(1, worker1Ids),
        processWorker(2, worker2Ids)
    ]);

    console.log(`\n\n🎉 Dual-Worker Hydration Complete!`);
    console.log(`Total AI Optimizations: ${globalAiCount}`);
}

main();
