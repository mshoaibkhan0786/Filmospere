...
export default function MovieListPage() {
    ...
    const fetchMovies = async () => {
        setIsLoading(true);
        // Admin needs to see LIST of movies. 
        // We select minimal data to make it fast.
        const { data, error } = await supabase
            .from('movies')
            .select('id, data->title, data->releaseYear, data->director, data->posterUrl, data->rating');
        ...
        if (error) {
            ...
            // Sort by latest added (approx by ID or title if no date, but likely better to fetch created_at)
            // But for now, simple sort by title or just raw
            setMovies(mapped);
        }
        setIsLoading(false);
    };
    ...
    const confirmDelete = async () => {
        if (deleteModal.movieId) {
            const { error } = await supabase.from('movies').delete().eq('id', deleteModal.movieId);
            if (error) {
    ...
    if (isLoading) return <div style={{ color: 'white', padding: '2rem' }}>Loading movies...</div>;

    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <h1 style={{ fontSize: '2rem', margin: 0, fontWeight: 'bold' }}>Manage Movies</h1>
                <Link
