...
const AdminArticleEditor: React.FC = () => {
    const { slug } = useParams<{ slug: string }>(); // If editing
    ...
    const [relatedMovieId, setRelatedMovieId] = useState('');
    ...
    const loadArticle = async (slugToLoad: string) => {
        setIsLoading(true);
        const data = await ArticleService.getArticleBySlug(slugToLoad);
        if (data) {
            setTitle(data.title);
            ...
            setRelatedMovieId(data.related_movie_id || '');
        }
        setIsLoading(false);
    };
    ...
    const handleSave = async () => {
        ...
        if (isEditing && slug) {
            const updated = await ArticleService.updateArticle(slug, articleData);
            if (updated) {
    ...
    if (isLoading) return <div style={{ color: 'white', padding: '2rem' }}>Loading editor...</div>;
        ...
        <div style={{ height: 'calc(100vh - 4rem)', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexShrink: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <button onClick={() => navigate('/admin/articles')} style={{ background: 'none', border: 'none', color: '#999', cursor: 'pointer' }}>
                        <ArrowLeft size={24} />
                    </button>
